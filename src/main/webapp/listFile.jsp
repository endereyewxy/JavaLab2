...
<div id="topbar">
    ...
    <div id="flowbar">
        <div id="crumbbar">
            <%
                String[] splitPath = ("files" + path).split("/");
                String cur_path = "/";
            %>
            <%
                for (int i = 0; i < splitPath.length; i++) {
                    cur_path += URLEncoder.encode(splitPath[i], "UTF-8") + "/";
            %>
            <a class="crumb<%=i==splitPath.length-1?" active":"" %>" href="<%=cur_path%>">
                <img class="sep" src="/resource/img/crumb.svg" alt=">">
                <span class="label"><%=StringEscapeUtils.escapeHtml4(splitPath[i])%></span>
            </a>
            <%
                }
            %>
    ...
    <div id="content">
        ...
        <div id="view" class="view-details view-size-20 width-1">
            <ul id="items" class="clearfix">
                ...
                %>
                <%
                    for (File x : (File[]) request.getAttribute("files")) {
                ...
                <li class="item <%=type%>">
                    <a href="<%=URLEncoder.encode(x.getName(),"UTF-8")+(x.isDirectory()?"/":"")%>">
                        ...
                        <span class="label"
                              title="<%=StringEscapeUtils.escapeHtml4(x.getName())%>"><%=StringEscapeUtils.escapeHtml4(x.getName())%></span>
                    </a>
                </li>
                <%
                    }
